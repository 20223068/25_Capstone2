...
또한 독후 활동 문제와 학습 피드백을 자동 생성하여 아동의 이해도, 표현력, 창의성을 평가하고 독서 학습 경험을 확장할 수 있도록 구현
...
본 프로젝트는 동화를 읽으며 아동이 질문을 하고, LLM이 동화 문맥을 기반으로 답변을 제공하는 **상호작용형 동화 학습 시스템**이다.

질문 임베딩과 유사도 기반 검색을 활용하여 반복 질문에는 빠르게 응답하고, 새로운 질문에는 관련 동화 문단을 함께 전달하여 LLM이 문맥 기반 답변을 생성하도록 설계하였다.

또한 독후 활동 문제와 학습 피드백을 자동 생성하여 아동의 이해도, 표현력, 창의성을 평가하고 독서 학습 경험을 확장할 수 있도록 구현하였다.
...
본 시스템은 **사용자 인터페이스(UI), 백엔드 서버, 데이터베이스(DB), LLM 모듈**로 구성된다.
...
이 구조를 통해 **동화 읽기 → 질문 → 답변 → 독후 활동 → 피드백**의 학습 흐름을 하나의 상호작용 시스템으로 통합하였다.
